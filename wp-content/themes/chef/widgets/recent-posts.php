...
class Chef_Recent_Posts extends WP_Widget {
...
// constructor
	...
	<p><label for="<?php echo $this->get_field_id( 'number' ); ?>"><?php _e( 'Number of posts to show:', 'chef' ); ?></label>
	...
	function widget($args, $instance) {
		$cache = array();
		if ( ! $this->is_preview() ) {
			$cache = wp_cache_get( 'chef_recent_posts', 'widget' );
		}
		...
		$title = ( ! empty( $instance['title'] ) ) ? $instance['title'] : __( 'Recent Posts', 'chef' );
...
?>
	...
	<?php
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'chef_recent_posts', $cache, 'widget' );
		} else {
			ob_end_flush();
		}
	}
